...
class PropertyChangeTest {
    ...
    @Test
    fun propertyChange() = runPropertyTest {
    ...
    @Test
    fun substituteActions() = runPropertyTest {
        ...
        val actions: List<ICodeCompletionAction> = cell.backend().getSubstituteActions().flatMap { it.flattenApplicableActions(parameters) }.toList()
        actions.forEach { println(it.getCompletionPattern() + " | " + it.getDescription()) }
        assertTrue(actions.isNotEmpty())
    }

    private fun runPropertyTest(body: suspend () -> Unit) = runTest {
        ...
        editor.flushAndUpdateSelection {
            val cell = checkNotNull(editor.resolvePropertyCell(C_NumberLiteral.value, numberLiteral)) {
                "Cell for property 'value' not found"
            }
            val layoutable = checkNotNull(cell.layoutable()) {
