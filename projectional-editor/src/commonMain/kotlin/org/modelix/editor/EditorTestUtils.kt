...
object EditorTestUtils {
    ...
    fun buildCells(template: Any, tree: IMutableCellTree): MutableCell {
        return when (template) {
            is IMutableCellTree.MutableCell -> template
            noSpace -> tree.createCell().apply { setProperty(CommonCellProperties.noSpace, true) }
            newLine -> tree.createCell().apply { setProperty(CommonCellProperties.onNewLine, true) }
            is String -> tree.createCell().apply {
    ...
    fun buildRandomCells(rand: Random, cellsPerLevel: Int, levels: Int, tree: IMutableCellTree): MutableCell {
        return buildCells(buildRandomTemplate(rand, cellsPerLevel, levels), tree)
    }
