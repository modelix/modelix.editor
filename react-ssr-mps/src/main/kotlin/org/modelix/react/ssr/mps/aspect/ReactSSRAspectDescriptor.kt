...
class ReactSSRAspectDescriptors : Disposable {
    private val descriptorCache: DescriptorCache<IReactSSRAspectDescriptor> =
        DescriptorCache(IReactSSRAspectDescriptor::class.java).also {
    ...
    fun findDescriptors(repository: SRepository): List<IReactSSRAspectDescriptor> =
        repository.computeRead {
            repository.modules.mapNotNull { descriptorCache.getDescriptor(it, "modelix.ReactDescriptor") }
        }
}

data class CompositeReactSSRAspectDescriptor(
    val descriptors: Set<IReactSSRAspectDescriptor>,
) : IReactSSRAspectDescriptor {
    override fun getRenderersForConcept(concept: ConceptReference): List<IReactNodeRenderer> =
        descriptors.flatMap { it.getRenderersForConcept(concept) }

    override fun getRenderers(signature: RendererSignature): List<IReactNodeRenderer> = descriptors.flatMap { it.getRenderers(signature) }
...
abstract class ReactSSRAspectDescriptorBase : IReactSSRAspectDescriptor {
    ...
    override fun getRenderersForConcept(concept: ConceptReference): List<IReactNodeRenderer> =
        getRenderers(ConceptRendererSignature(concept))

    override fun getRenderers(signature: RendererSignature): List<IReactNodeRenderer> = renderers[signature] ?: emptyList()

    protected fun addRenderer(
        concept: ConceptReference,
        renderer: IReactNodeRenderer,
    ) {
    ...
    override fun getPages(): List<IReactPageDescriptor> = pages
